...
class SentimentIntensityAnalyzer(object):
    ...
    # def __init__(self, lexicon_file="data/combined_lexicon2.txt"):
        ...
        emoji_lex_dic = {}
        with io.open(os.path.join(dirname(_this_module_file_path_),
                                  'lexicons/emoji_lexicon.txt'), encoding='utf-16') as f:
...
def main():
    ...
    logger.info("Loading article body and compute sentiments.")
    ...
    for article in os.listdir(articles_dir):
        ...
        if article_id.isdigit():
            ...
            if paragraphs:
                ...
                # Compute probability for sentiment classes
                article_sentiment = {}
                article_sentiment['article_id'] = int(article_id)
                article_sentiment['neg'] = classifier.polarity_scores(article_body)["neg"]
                article_sentiment['neu'] = classifier.polarity_scores(article_body)["neu"]
                article_sentiment['pos'] = classifier.polarity_scores(article_body)["pos"]
                article_sentiment['compound'] = classifier.polarity_scores(article_body)["compound"]
                senti_probs.append(article_sentiment)

            article_count += 1
            if article_count % 100 == 0:
                logger.info("Sentiment of %s out of %s articles has been calculated.",
                            article_count, n_articles)

    # Write sentiment probability and scores into file
